...
- 保存先はドライブのアプリ専用領域（appDataFolder）の `kakeibo-data.json`。マイドライブには表示されない非公開領域で、このアプリだけが読み書きでき、逆にアプリはドライブ内の他のファイルにアクセスできません
- 保存データの確認・削除は、Googleドライブの「設定 > アプリの管理」から行えます（「非表示のアプリデータを削除」でドライブ側のデータを完全消去）
